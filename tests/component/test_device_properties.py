...
def test___constructed_device___get_property___returns_value(generate_device):
    device = generate_device("bridgeTester")

    assert device.product_type == "PXIe-4331"


def test___nonexistent_device___get_property___raises_invalid_device_id(generate_device):
    device = generate_device("NonexistentDevice")
...
def test___devices_with_same_name___compare___equal(generate_device):
    device1 = generate_device("bridgeTester")
    device2 = generate_device("bridgeTester")

    assert device1 is not device2
    assert device1 == device2


def test___devices_with_different_names___compare___not_equal(generate_device):
    device1 = generate_device("bridgeTester")
    device2 = generate_device("tsVoltageTester1")
...
@pytest.mark.device_name("bridgeTester")
def test___device___get_deprecated_properties___reports_warnings(device: Device):
