...
class GrpcEventHandler(BaseEventHandler):
    def __init__(self, response_stream, thread):
...
class GrpcStubInterpreter(BaseInterpreter):
    ...
    def register_done_event(
        ...
        if callback_function is not None:
            return self._register_done_event(task, options, callback_function, callback_data)
        else:
            raise NotImplementedError("The NI gRPC Device Server does not support event unregistration.")
    ...
    def _register_done_event(
        ...
        thread = threading.Thread(target=done_event_thread, name="NI-DAQmx Done Event Thread")
        thread.start()
        return GrpcEventHandler(response_stream, thread)
    ...
    def register_every_n_samples_event(
        ...
        if callback_function is not None:
            return self._register_every_n_samples_event(task, every_n_samples_event_type, n_samples, options, callback_function, callback_data)
        else:
            raise NotImplementedError("The NI gRPC Device Server does not support event unregistration.")
    ...
    def _register_every_n_samples_event(
        ...
        thread = threading.Thread(target=every_n_samples_event_thread, name="NI-DAQmx Every N Samples Event Thread")
        thread.start()
        return GrpcEventHandler(response_stream, thread)
