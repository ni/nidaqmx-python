...
class Timing(object):
    ...
    def first_samp_clk_offset(self):
        ...
        cfunc = lib_importer.windll.DAQmxGetFirstSampClkOffset
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_clk_offset(self, val):
        cfunc = lib_importer.windll.DAQmxSetFirstSampClkOffset
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_clk_offset(self):
        cfunc = lib_importer.windll.DAQmxResetFirstSampClkOffset
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_clk_timescale(self):
        ...
        cfunc = lib_importer.windll.DAQmxGetFirstSampClkTimescale
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_clk_timescale(self, val):
        val = val.value
        cfunc = lib_importer.windll.DAQmxSetFirstSampClkTimescale
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_clk_timescale(self):
        cfunc = lib_importer.windll.DAQmxResetFirstSampClkTimescale
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_clk_when(self):
        """
        unknown: Specifies the time of the first sample clock pulse.
        """
        val = ctypes.CVIAbsoluteTime()

        cfunc = lib_importer.windll.DAQmxGetFirstSampClkWhen
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
                        lib_importer.task_handle,
                        ctypes.POINTER(ctypes.CVIAbsoluteTime)]
    ...
    def first_samp_clk_when(self, val):
        cfunc = lib_importer.windll.DAQmxSetFirstSampClkWhen
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_clk_when(self):
        cfunc = lib_importer.windll.DAQmxResetFirstSampClkWhen
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def first_samp_timestamp_val(self):
        """
        unknown: Indicates the timestamp of the first sample.
        """
        val = ctypes.CVIAbsoluteTime()

        cfunc = lib_importer.windll.DAQmxGetFirstSampTimestampVal
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def sync_pulse_time_when(self):
        """
        unknown: Specifies the start time of the sync pulse.
        """
        val = ctypes.CVIAbsoluteTime()

        cfunc = lib_importer.windll.DAQmxGetSyncPulseTimeWhen
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def sync_pulse_time_when(self, val):
        cfunc = lib_importer.windll.DAQmxSetSyncPulseTimeWhen
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    def sync_pulse_time_when(self):
        cfunc = lib_importer.windll.DAQmxResetSyncPulseTimeWhen
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    @property
    def timing_sync_pulse_force(self):
        """
        bool: Specifies whether a synchronization pulse should be
            generated at start regardless of whether devices in the task
            are synchronized. When set to false, a synchronization pulse
            will be generated only if all devices in the task are not
            synchronized. Some DSA AO devices have unexpected behavior
            during DAC synchronization. Refer to your device's
            documentation for more information.
        """
        val = c_bool32()

        cfunc = lib_importer.windll.DAQmxGetTimingSyncPulseForce
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    @timing_sync_pulse_force.setter
    def timing_sync_pulse_force(self, val):
        cfunc = lib_importer.windll.DAQmxSetTimingSyncPulseForce
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
    ...
    @timing_sync_pulse_force.deleter
    def timing_sync_pulse_force(self):
        cfunc = lib_importer.windll.DAQmxResetTimingSyncPulseForce
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
