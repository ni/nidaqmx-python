...
class LibraryInterpreter(BaseInterpreter):
    ...
    def __init__(self):
        ...
        if _was_runtime_environment_set is None:
            try:
                ...
                self.set_runtime_environment(runtime_env, version, "", "")
    ...
    def add_global_chans_to_task(self, task, channel_names):
        ...
        error_code = cfunc(task, channel_names)
        self.check_for_error(error_code)
    ...
    def add_network_device(self, ip_address, device_name, attempt_reservation, timeout):
        ...
        while True:
            ...
            size_or_code = cfunc(
                ip_address, device_name, attempt_reservation, timeout, device_name_out, temp_size
            )
    ...
    def are_configured_cdaq_sync_ports_disconnected(self, chassis_devices_ports, timeout):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [ctypes_byte_str, ctypes.c_double, ctypes.POINTER(c_bool32)]

        error_code = cfunc(chassis_devices_ports, timeout, ctypes.byref(disconnected_ports_exist))
        self.check_for_error(error_code)
    ...
    def auto_configure_cdaq_sync_connections(self, chassis_devices_ports, timeout):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [ctypes_byte_str, ctypes.c_double]

        error_code = cfunc(chassis_devices_ports, timeout)
        self.check_for_error(error_code)

    def calculate_reverse_poly_coeff(
        self, forward_coeffs, min_val_x, max_val_x, num_points_to_compute, reverse_poly_order
    ):
    ...
    def cfg_anlg_edge_ref_trig(
        self, task, trigger_source, pretrigger_samples, trigger_slope, trigger_level
    ):
        ...
        error_code = cfunc(task, trigger_source, trigger_slope, trigger_level, pretrigger_samples)
        self.check_for_error(error_code)
    ...
    def cfg_anlg_edge_start_trig(self, task, trigger_source, trigger_slope, trigger_level):
        ...
        error_code = cfunc(task, trigger_source, trigger_slope, trigger_level)
        self.check_for_error(error_code)
    ...
    def cfg_anlg_multi_edge_start_trig(
        self, task, trigger_sources, trigger_slope_array, trigger_level_array
    ):
    ...
    def cfg_anlg_window_ref_trig(
        self, task, trigger_source, window_top, window_bottom, pretrigger_samples, trigger_when
    ):
        ...
        error_code = cfunc(
            task, trigger_source, trigger_when, window_top, window_bottom, pretrigger_samples
        )
        self.check_for_error(error_code)

    def cfg_anlg_window_start_trig(
        self, task, window_top, window_bottom, trigger_source, trigger_when
    ):
        ...
        error_code = cfunc(task, trigger_source, trigger_when, window_top, window_bottom)
        self.check_for_error(error_code)
    ...
    def cfg_change_detection_timing(
        self, task, rising_edge_chan, falling_edge_chan, sample_mode, samps_per_chan
    ):
        ...
        error_code = cfunc(task, rising_edge_chan, falling_edge_chan, sample_mode, samps_per_chan)
        self.check_for_error(error_code)
    ...
    def cfg_dig_edge_ref_trig(self, task, trigger_source, pretrigger_samples, trigger_edge):
        ...
        error_code = cfunc(task, trigger_source, trigger_edge, pretrigger_samples)
        self.check_for_error(error_code)
    ...
    def cfg_dig_edge_start_trig(self, task, trigger_source, trigger_edge):
        ...
        error_code = cfunc(task, trigger_source, trigger_edge)
        self.check_for_error(error_code)

    def cfg_dig_pattern_ref_trig(
        self, task, trigger_source, trigger_pattern, pretrigger_samples, trigger_when
    ):
        ...
        error_code = cfunc(task, trigger_source, trigger_pattern, trigger_when, pretrigger_samples)
        self.check_for_error(error_code)
    ...
    def cfg_dig_pattern_start_trig(self, task, trigger_source, trigger_pattern, trigger_when):
        ...
        error_code = cfunc(task, trigger_source, trigger_pattern, trigger_when)
        self.check_for_error(error_code)
    ...
    def cfg_handshaking_timing(self, task, sample_mode, samps_per_chan):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes.c_int, ctypes.c_ulonglong]

        error_code = cfunc(task, sample_mode, samps_per_chan)
        self.check_for_error(error_code)
    ...
    def cfg_pipelined_samp_clk_timing(
        self, task, rate, source, active_edge, sample_mode, samps_per_chan
    ):
        ...
        error_code = cfunc(task, source, rate, active_edge, sample_mode, samps_per_chan)
        self.check_for_error(error_code)
    ...
    def cfg_time_start_trig(self, task, when, timescale):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, AbsoluteTime, ctypes.c_int]

        error_code = cfunc(task, AbsoluteTime.from_datetime(when), timescale)
        self.check_for_error(error_code)
    ...
    def cfg_watchdog_ao_expir_states(
        ...
        error_code = cfunc(
            task, channel_names, expir_state_array, output_type_array, len(output_type_array)
        )
        self.check_for_error(error_code)
    ...
    def cfg_watchdog_co_expir_states(self, task, channel_names, expir_state_array):
        ...
        error_code = cfunc(task, channel_names, expir_state_array, len(expir_state_array))
        self.check_for_error(error_code)
    ...
    def clear_teds(self, physical_channel):
        ...
        error_code = cfunc(physical_channel)
        self.check_for_error(error_code)
    ...
    def configure_logging(self, task, file_path, logging_mode, group_name, operation):
        ...
        error_code = cfunc(task, file_path, logging_mode, group_name, operation)
        self.check_for_error(error_code)
    ...
    def configure_teds(self, physical_channel, file_path):
        ...
        error_code = cfunc(physical_channel, file_path)
        self.check_for_error(error_code)
    ...
    def connect_terms(self, source_terminal, destination_terminal, signal_modifiers):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [ctypes_byte_str, ctypes_byte_str, ctypes.c_int]

        error_code = cfunc(source_terminal, destination_terminal, signal_modifiers)
        self.check_for_error(error_code)
    ...
    def create_ai_temp_built_in_sensor_chan(
        ...
        error_code = cfunc(task, physical_channel, name_to_assign_to_channel, units)
        self.check_for_error(error_code)
    ...
    def create_ao_func_gen_chan(
        self, task, physical_channel, name_to_assign_to_channel, type, freq, amplitude, offset
    ):
        ...
        error_code = cfunc(
            task, physical_channel, name_to_assign_to_channel, type, freq, amplitude, offset
        )
        self.check_for_error(error_code)
    ...
    def create_ci_count_edges_chan(
        self, task, counter, name_to_assign_to_channel, edge, initial_count, count_direction
    ):
        ...
        error_code = cfunc(
            task, counter, name_to_assign_to_channel, edge, initial_count, count_direction
        )
        self.check_for_error(error_code)

    def create_ci_duty_cycle_chan(
        self, task, counter, name_to_assign_to_channel, min_freq, max_freq, edge, custom_scale_name
    ):
        ...
        error_code = cfunc(
            task, counter, name_to_assign_to_channel, min_freq, max_freq, edge, custom_scale_name
        )
        self.check_for_error(error_code)
    ...
    def create_ci_pulse_chan_freq(
        self, task, counter, name_to_assign_to_channel, min_val, max_val, units
    ):
        ...
        error_code = cfunc(task, counter, name_to_assign_to_channel, min_val, max_val, units)
        self.check_for_error(error_code)

    def create_ci_pulse_chan_ticks(
        self, task, counter, name_to_assign_to_channel, source_terminal, min_val, max_val
    ):
        ...
        error_code = cfunc(
            task, counter, name_to_assign_to_channel, source_terminal, min_val, max_val
        )
        self.check_for_error(error_code)
    ...
    def create_ci_semi_period_chan(
        self, task, counter, name_to_assign_to_channel, min_val, max_val, units, custom_scale_name
    ):
        ...
        error_code = cfunc(
            task, counter, name_to_assign_to_channel, min_val, max_val, units, custom_scale_name
        )
        self.check_for_error(error_code)
    ...
    def create_cigps_timestamp_chan(
        self, task, counter, name_to_assign_to_channel, units, sync_method, custom_scale_name
    ):
        ...
        error_code = cfunc(
            task, counter, name_to_assign_to_channel, units, sync_method, custom_scale_name
        )
        self.check_for_error(error_code)
    ...
    def create_di_chan(self, task, lines, name_to_assign_to_lines, line_grouping):
        ...
        error_code = cfunc(task, lines, name_to_assign_to_lines, line_grouping)
        self.check_for_error(error_code)
    ...
    def create_lin_scale(self, name, slope, y_intercept, pre_scaled_units, scaled_units):
        ...
        error_code = cfunc(name, slope, y_intercept, pre_scaled_units, scaled_units)
        self.check_for_error(error_code)
    ...
    def create_polynomial_scale(
        self, name, forward_coeffs, reverse_coeffs, pre_scaled_units, scaled_units
    ):
    ...
    def create_task(self, session_name):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [ctypes_byte_str, ctypes.POINTER(lib_importer.task_handle)]

        error_code = cfunc(session_name, ctypes.byref(task))
        self.check_for_error(error_code)
    ...
    def create_watchdog_timer_task_ex(self, device_name, session_name, timeout):
        ...
        error_code = cfunc(device_name, session_name, ctypes.byref(task), timeout)
        self.check_for_error(error_code)
        return task, new_session_initialized
    ...
    def device_supports_cal(self, device_name):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [ctypes_byte_str, ctypes.POINTER(c_bool32)]

        error_code = cfunc(device_name, ctypes.byref(cal_supported))
        self.check_for_error(error_code)
    ...
    def disconnect_terms(self, source_terminal, destination_terminal):
        ...
        error_code = cfunc(source_terminal, destination_terminal)
        self.check_for_error(error_code)
    ...
    def export_signal(self, task, signal_id, output_terminal):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes.c_int, ctypes_byte_str]

        error_code = cfunc(task, signal_id, output_terminal)
        self.check_for_error(error_code)
    ...
    def get_analog_power_up_states_with_output_type(self, channel_names, array_size):
        ...
        error_code = cfunc(channel_names, state_array, channel_type_array, ctypes.byref(array_size))
        self.check_for_error(error_code)
    ...
    def get_auto_configured_cdaq_sync_connections(self):
        ...
        while True:
            ...
            size_or_code = cfunc(port_list, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_buffer_attribute_uint32(self, task, attribute):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes.c_int32]

        error_code = cfunc(task, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_cal_info_attribute_bool(self, device_name, attribute):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [ctypes_byte_str, ctypes.c_int32]

        error_code = cfunc(device_name, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_cal_info_attribute_string(self, device_name, attribute):
        ...
        while True:
            ...
            size_or_code = cfunc(device_name, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_chan_attribute_bool(self, task, channel, attribute):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes_byte_str, ctypes.c_int32]

        error_code = cfunc(task, channel, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_chan_attribute_double_array(self, task, channel, attribute):
        ...
        while True:
            ...
            size_or_code = cfunc(
                task, channel, attribute, value.ctypes.data_as(ctypes.c_void_p), temp_size
            )
    ...
    def get_chan_attribute_string(self, task, channel, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(task, channel, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_device_attribute_double_array(self, device_name, attribute):
        ...
        while True:
            value = numpy.zeros(temp_size, dtype=numpy.float64)
            size_or_code = cfunc(
                device_name, attribute, value.ctypes.data_as(ctypes.c_void_p), temp_size
            )
            if is_array_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
    ...
    def get_digital_logic_family_power_up_state(self, device_name):
        ...
        error_code = cfunc(device_name, ctypes.byref(logic_family))
        self.check_for_error(error_code)
    ...
    def get_exported_signal_attribute_string(self, task, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(task, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_persisted_chan_attribute_bool(self, channel, attribute):
        ...
        error_code = cfunc(channel, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_persisted_chan_attribute_string(self, channel, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(channel, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_persisted_scale_attribute_bool(self, scale_name, attribute):
        ...
        error_code = cfunc(scale_name, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_persisted_scale_attribute_string(self, scale_name, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(scale_name, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_persisted_task_attribute_bool(self, task_name, attribute):
        ...
        error_code = cfunc(task_name, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_persisted_task_attribute_string(self, task_name, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(task_name, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_physical_chan_attribute_bool(self, physical_channel, attribute):
        ...
        error_code = cfunc(physical_channel, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_physical_chan_attribute_bytes(self, physical_channel, attribute):
        ...
        while True:
            ...
            size_or_code = cfunc(
                physical_channel, attribute, value.ctypes.data_as(ctypes.c_void_p), temp_size
            )
            if is_array_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
    ...
    def get_physical_chan_attribute_string(self, physical_channel, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(physical_channel, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_scale_attribute_double_array(self, scale_name, attribute):
        ...
        while True:
            value = numpy.zeros(temp_size, dtype=numpy.float64)
            size_or_code = cfunc(
                scale_name, attribute, value.ctypes.data_as(ctypes.c_void_p), temp_size
            )
            if is_array_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
    ...
    def get_system_info_attribute_string(self, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_system_info_attribute_uint32(self, attribute):
        ...
        error_code = cfunc(attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_timing_attribute_ex_bool(self, task, device_names, attribute):
        ...
        error_code = cfunc(task, device_names, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_timing_attribute_ex_string(self, task, device_names, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(task, device_names, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_trig_attribute_double_array(self, task, attribute):
        ...
        while True:
            value = numpy.zeros(temp_size, dtype=numpy.float64)
            size_or_code = cfunc(task, attribute, value.ctypes.data_as(ctypes.c_void_p), temp_size)
            if is_array_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def get_watchdog_attribute_bool(self, task, lines, attribute):
        ...
        error_code = cfunc(task, lines, attribute, ctypes.byref(value))
        self.check_for_error(error_code)
        return value.value
    ...
    def get_watchdog_attribute_string(self, task, lines, attribute):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(task, lines, attribute, value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def internal_get_last_created_chan(self):
        ...
        while True:
            value = ctypes.create_string_buffer(temp_size)
            size_or_code = cfunc(value, temp_size)
            if is_string_buffer_too_small(size_or_code):
                # Buffer size must have changed between calls; check again.
                temp_size = 0
            elif size_or_code > 0 and temp_size == 0:
    ...
    def is_task_done(self, task):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes.POINTER(c_bool32)]

        error_code = cfunc(task, ctypes.byref(is_task_done))
        self.check_for_error(error_code)
    ...
    def perform_bridge_offset_nulling_cal_ex(self, task, channel, skip_unsupported_channels):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [lib_importer.task_handle, ctypes_byte_str, c_bool32]

        error_code = cfunc(task, channel, skip_unsupported_channels)
        self.check_for_error(error_code)
    ...
    def read_analog_scalar_f64(self, task, timeout):
        ...
        error_code = cfunc(task, timeout, ctypes.byref(value), None)
        self.check_for_error(error_code)
        return value.value
    ...
    def read_ctr_freq_scalar(self, task, timeout):
        ...
        error_code = cfunc(task, timeout, ctypes.byref(frequency), ctypes.byref(duty_cycle), None)
        self.check_for_error(error_code)
    ...
    def read_ctr_ticks(
        ...
        timeout,
        interleaved,
        read_array_high_ticks,
        read_array_low_ticks,
    ):
        samps_per_chan_read = ctypes.c_int()
    ...
    def read_ctr_ticks_scalar(self, task, timeout):
        ...
        error_code = cfunc(task, timeout, ctypes.byref(high_ticks), ctypes.byref(low_ticks), None)
        self.check_for_error(error_code)
    ...
    def read_ctr_time_scalar(self, task, timeout):
        ...
        error_code = cfunc(task, timeout, ctypes.byref(high_time), ctypes.byref(low_time), None)
        self.check_for_error(error_code)
    ...
    def read_power_scalar_f64(self, task, timeout):
        ...
        error_code = cfunc(task, timeout, ctypes.byref(voltage), ctypes.byref(current), None)
        self.check_for_error(error_code)
    ...
    def register_done_event(self, task, options, callback_function, callback_data):
        DAQmxDoneEventCallbackPtr = ctypes.CFUNCTYPE(
            ctypes.c_int32, lib_importer.task_handle, ctypes.c_int, ctypes.c_void_p
        )
    ...
    def register_every_n_samples_event(
        self, task, every_n_samples_event_type, n_samples, options, callback_function, callback_data
    ):
        DAQmxEveryNSamplesEventCallbackPtr = ctypes.CFUNCTYPE(
            ctypes.c_int32, lib_importer.task_handle, ctypes.c_int, ctypes.c_uint, ctypes.c_void_p
        )
        ...
        error_code = cfunc(
            task, every_n_samples_event_type, n_samples, options, callback_method_ptr, callback_data
        )
        self.check_for_error(error_code)
    ...
    def register_signal_event(self, task, signal_id, options, callback_function, callback_data):
        ...
        error_code = cfunc(task, signal_id, options, callback_method_ptr, callback_data)
        self.check_for_error(error_code)
    ...
    def reserve_network_device(self, device_name, override_reservation):
        ...
        if cfunc.argtypes is None:
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [ctypes_byte_str, c_bool32]

        error_code = cfunc(device_name, override_reservation)
        self.check_for_error(error_code)
    ...
    def reset_buffer_attribute(self, task, attribute):
        ...
        error_code = cfunc(task, attribute)
        self.check_for_error(error_code)
    ...
    def reset_chan_attribute(self, task, channel, attribute):
        ...
        error_code = cfunc(task, channel, attribute)
        self.check_for_error(error_code)
    ...
    def reset_timing_attribute_ex(self, task, device_names, attribute):
        ...
        error_code = cfunc(task, device_names, attribute)
        self.check_for_error(error_code)
    ...
    def reset_watchdog_attribute(self, task, lines, attribute):
        ...
        error_code = cfunc(task, lines, attribute)
        self.check_for_error(error_code)
    ...
    def save_global_chan(self, task, channel_name, save_as, author, options):
        ...
        error_code = cfunc(task, channel_name, save_as, author, options)
        self.check_for_error(error_code)
    ...
    def save_scale(self, scale_name, save_as, author, options):
        ...
        error_code = cfunc(scale_name, save_as, author, options)
        self.check_for_error(error_code)
    ...
    def save_task(self, task, save_as, author, options):
        ...
        error_code = cfunc(task, save_as, author, options)
        self.check_for_error(error_code)
    ...
    def set_analog_power_up_states_with_output_type(
        ...
        error_code = cfunc(channel_names, state_array, channel_type_array, len(channel_type_array))
        self.check_for_error(error_code)
    ...
    def set_buffer_attribute_uint32(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, ctypes.c_uint32(value))
        self.check_for_error(error_code)
    ...
    def set_cal_info_attribute_bool(self, device_name, attribute, value):
        ...
        error_code = cfunc(device_name, attribute, c_bool32(value))
        self.check_for_error(error_code)
    ...
    def set_cal_info_attribute_double(self, device_name, attribute, value):
        ...
        error_code = cfunc(device_name, attribute, ctypes.c_double(value))
        self.check_for_error(error_code)
    ...
    def set_cal_info_attribute_string(self, device_name, attribute, value):
        ...
        error_code = cfunc(device_name, attribute, value.encode(lib_importer.encoding))
        self.check_for_error(error_code)
    ...
    def set_cal_info_attribute_uint32(self, device_name, attribute, value):
        ...
        error_code = cfunc(device_name, attribute, ctypes.c_uint32(value))
        self.check_for_error(error_code)
    ...
    def set_chan_attribute_bool(self, task, channel, attribute, value):
        ...
        error_code = cfunc(task, channel, attribute, c_bool32(value))
        self.check_for_error(error_code)
    ...
    def set_chan_attribute_double(self, task, channel, attribute, value):
        ...
        error_code = cfunc(task, channel, attribute, ctypes.c_double(value))
        self.check_for_error(error_code)
    ...
    def set_chan_attribute_double_array(self, task, channel, attribute, value):
        ...
        error_code = cfunc(
            task, channel, attribute, value.ctypes.data_as(ctypes.c_void_p), len(value)
        )
        self.check_for_error(error_code)
    ...
    def set_chan_attribute_int32(self, task, channel, attribute, value):
        ...
        error_code = cfunc(task, channel, attribute, ctypes.c_int32(value))
        self.check_for_error(error_code)
    ...
    def set_chan_attribute_string(self, task, channel, attribute, value):
        ...
        error_code = cfunc(task, channel, attribute, value.encode(lib_importer.encoding))
        self.check_for_error(error_code)
    ...
    def set_chan_attribute_uint32(self, task, channel, attribute, value):
        ...
        error_code = cfunc(task, channel, attribute, ctypes.c_uint32(value))
        self.check_for_error(error_code)
    ...
    def set_digital_logic_family_power_up_state(self, device_name, logic_family):
        ...
        error_code = cfunc(device_name, logic_family)
        self.check_for_error(error_code)
    ...
    def set_exported_signal_attribute_bool(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, c_bool32(value))
        self.check_for_error(error_code)
    ...
    def set_exported_signal_attribute_double(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, ctypes.c_double(value))
        self.check_for_error(error_code)
    ...
    def set_exported_signal_attribute_int32(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, ctypes.c_int32(value))
        self.check_for_error(error_code)
    ...
    def set_exported_signal_attribute_string(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, value.encode(lib_importer.encoding))
        self.check_for_error(error_code)
    ...
    def set_read_attribute_uint64(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, ctypes.c_uint64(value))
        self.check_for_error(error_code)
    ...
    def set_runtime_environment(self, environment, environment_version, reserved_1, reserved_2):
        ...
        error_code = cfunc(environment, environment_version, reserved_1, reserved_2)
        self.check_for_error(error_code)
    ...
    def set_scale_attribute_double(self, scale_name, attribute, value):
        ...
        error_code = cfunc(scale_name, attribute, ctypes.c_double(value))
        self.check_for_error(error_code)
    ...
    def set_scale_attribute_double_array(self, scale_name, attribute, value):
        ...
        error_code = cfunc(scale_name, attribute, value.ctypes.data_as(ctypes.c_void_p), len(value))
        self.check_for_error(error_code)
    ...
    def set_scale_attribute_int32(self, scale_name, attribute, value):
        ...
        error_code = cfunc(scale_name, attribute, ctypes.c_int32(value))
        self.check_for_error(error_code)
    ...
    def set_scale_attribute_string(self, scale_name, attribute, value):
        ...
        error_code = cfunc(scale_name, attribute, value.encode(lib_importer.encoding))
        self.check_for_error(error_code)
    ...
    def set_timing_attribute_ex_bool(self, task, device_names, attribute, value):
        ...
        error_code = cfunc(task, device_names, attribute, c_bool32(value))
        self.check_for_error(error_code)
    ...
    def set_timing_attribute_ex_double(self, task, device_names, attribute, value):
        ...
        error_code = cfunc(task, device_names, attribute, ctypes.c_double(value))
        self.check_for_error(error_code)
    ...
    def set_timing_attribute_ex_int32(self, task, device_names, attribute, value):
        ...
        error_code = cfunc(task, device_names, attribute, ctypes.c_int32(value))
        self.check_for_error(error_code)
    ...
    def set_timing_attribute_ex_string(self, task, device_names, attribute, value):
        ...
        error_code = cfunc(task, device_names, attribute, value.encode(lib_importer.encoding))
        self.check_for_error(error_code)
    ...
    def set_timing_attribute_ex_uint32(self, task, device_names, attribute, value):
        ...
        error_code = cfunc(task, device_names, attribute, ctypes.c_uint32(value))
        self.check_for_error(error_code)
    ...
    def set_timing_attribute_ex_uint64(self, task, device_names, attribute, value):
        ...
        error_code = cfunc(task, device_names, attribute, ctypes.c_uint64(value))
        self.check_for_error(error_code)
    ...
    def set_trig_attribute_double_array(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, value.ctypes.data_as(ctypes.c_void_p), len(value))
        self.check_for_error(error_code)
    ...
    def set_trig_attribute_timestamp(self, task, attribute, value):
        ...
        error_code = cfunc(task, attribute, AbsoluteTime.from_datetime(value))
        self.check_for_error(error_code)
    ...
    def set_watchdog_attribute_bool(self, task, lines, attribute, value):
        ...
        error_code = cfunc(task, lines, attribute, c_bool32(value))
        self.check_for_error(error_code)
    ...
    def set_watchdog_attribute_double(self, task, lines, attribute, value):
        ...
        error_code = cfunc(task, lines, attribute, ctypes.c_double(value))
        self.check_for_error(error_code)
    ...
    def set_watchdog_attribute_int32(self, task, lines, attribute, value):
        ...
        error_code = cfunc(task, lines, attribute, ctypes.c_int32(value))
        self.check_for_error(error_code)
    ...
    def set_watchdog_attribute_string(self, task, lines, attribute, value):
        ...
        error_code = cfunc(task, lines, attribute, value.encode(lib_importer.encoding))
        self.check_for_error(error_code)
    ...
    def start_new_file(self, task, file_path):
        ...
        error_code = cfunc(task, file_path)
        self.check_for_error(error_code)
    ...
    def tristate_output_term(self, output_terminal):
        ...
        error_code = cfunc(output_terminal)
        self.check_for_error(error_code)
            ...
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
                        ...
                        ctypes.c_int32,
                        ctypes.c_double,
                        ctypes.POINTER(AbsoluteTime),
                    ]

        error_code = cfunc(task, timestamp_event, timeout, ctypes.byref(timestamp))
        self.check_for_error(error_code)
    ...
    def wait_until_task_done(self, task, time_to_wait):
        ...
        error_code = cfunc(task, time_to_wait)
        self.check_for_error(error_code)
    ...
    def write_analog_scalar_f64(self, task, auto_start, timeout, value):
        ...
        error_code = cfunc(task, auto_start, timeout, value, None)
        self.check_for_error(error_code)
    ...
    def write_ctr_freq(
        self, task, num_samps_per_chan, auto_start, timeout, data_layout, frequency, duty_cycle
    ):
        ...
        error_code = cfunc(
            ...
            frequency,
            duty_cycle,
            ctypes.byref(num_samps_per_chan_written),
            None,
        )
    ...
    def write_ctr_freq_scalar(self, task, auto_start, timeout, frequency, duty_cycle):
        ...
        error_code = cfunc(task, auto_start, timeout, frequency, duty_cycle, None)
        self.check_for_error(error_code)

    def write_ctr_ticks(
        self, task, num_samps_per_chan, auto_start, timeout, data_layout, high_ticks, low_ticks
    ):
        num_samps_per_chan_written = ctypes.c_int()
    ...
    def write_ctr_ticks_scalar(self, task, auto_start, timeout, high_ticks, low_ticks):
        ...
        error_code = cfunc(task, auto_start, timeout, high_ticks, low_ticks, None)
        self.check_for_error(error_code)

    def write_ctr_time(
        self, task, num_samps_per_chan, auto_start, timeout, data_layout, high_time, low_time
    ):
        num_samps_per_chan_written = ctypes.c_int()
    ...
    def write_ctr_time_scalar(self, task, auto_start, timeout, high_time, low_time):
        ...
        error_code = cfunc(task, auto_start, timeout, high_time, low_time, None)
        self.check_for_error(error_code)
    ...
    def write_id_pin_memory(self, device_name, id_pin_name, data, format_code):
        ...
        error_code = cfunc(device_name, id_pin_name, data, len(data), format_code)
        self.check_for_error(error_code)
    ...
    def write_to_teds_from_array(self, physical_channel, bit_stream, basic_teds_options):
        ...
        error_code = cfunc(physical_channel, bit_stream, len(bit_stream), basic_teds_options)
        self.check_for_error(error_code)
    ...
    def write_to_teds_from_file(self, physical_channel, file_path, basic_teds_options):
        ...
        error_code = cfunc(physical_channel, file_path, basic_teds_options)
        self.check_for_error(error_code)
    ...
    def get_error_string(self, error_code):
        ...
        if query_error_code < 0:
            _logger.error(
                "Failed to get error string for error code %d. DAQmxGetErrorString returned error code %d.",
                error_code,
                query_error_code,
            )
    ...
    def get_extended_error_info(self):
        ...
        if query_error_code < 0:
            _logger.error(
                "Failed to get extended error info. DAQmxGetExtendedErrorInfo returned error code %d.",
                query_error_code,
            )
            return "Failed to retrieve error description."
        return error_buffer.value.decode(lib_importer.encoding)
    ...
    def read_id_pin_memory(self, device_name, id_pin_name):
        ...
        array_size = cfunc(
            ...
            None,
            0,
            ctypes.byref(data_length_read),
            ctypes.byref(format_code),
        )
    ...
    def read_raw(self, task, num_samps_per_chan, timeout, read_array):
        ...
        error_code = cfunc(
            ...
            ctypes.byref(number_of_bytes_per_sample),
            None,
        )
            ...
            with cfunc.arglock:
                if cfunc.argtypes is None:
                    cfunc.argtypes = [
                        ...
                        c_bool32,
                        ctypes.c_double,
                        wrapped_ndpointer(dtype=numpy_array.dtype, flags=("C")),
                        ctypes.POINTER(ctypes.c_int),
                        ctypes.POINTER(c_bool32),
                    ]
...
def is_string_buffer_too_small(error_code):
    return (
        error_code == DAQmxErrors.BUFFER_TOO_SMALL_FOR_STRING
        or error_code == DAQmxWarnings.CAPI_STRING_TRUNCATED_TO_FIT_BUFFER
    )
